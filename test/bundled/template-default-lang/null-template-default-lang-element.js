...
case 'mixin':
  {
    class NullTemplateDefaultLangElement extends Mixins.Logger(Mixins.Localizable(LegacyElementMixin(HTMLElement))) {
      ...
      static get template() {
        return ((t) => { t.setAttribute("lang", ""); return t; })(((t) => { t.setAttribute("localizable-text", "embedded"); return t; })(html`{{text.text}}<div><div></div></div><!-- nested empty div -->
    <span id="whitespace"> &nbsp; </span>
...
case 'legacy':
  {
    Polymer({
      ...
      _template: ((t) => { t.setAttribute("lang", ""); return t; })(((t) => { t.setAttribute("localizable-text", "embedded"); return t; })(html`{{text.text}}<div><div></div></div><!-- nested empty div -->
    <span id="whitespace"> &nbsp; </span>
    <h1>{{text.h1_3}}</h1>{{text.text_4}}<span>{{text.span_5}}</span>
    <span>{{text.span_6}}</span>
    <span id="label-1">{{text.label-1}}</span>
