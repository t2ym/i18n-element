...
'PolymerElement can create dom as children instead of in '+'ShadowDOM by setting `this.root = this;\` before \`ready\`.');}}/**
   ...
   */const html=function html(strings,...values){const template=/** @type {!HTMLTemplateElement} */document.createElement('template');template.innerHTML=values.reduce((acc,v,idx)=>acc+htmlValue(v)+strings[idx+1],strings[0]);return template;};/**
    ...
    */const htmlLiteral=function(strings,...values){return new LiteralString(values.reduce((acc,v,idx)=>acc+literalValue(v)+strings[idx+1],strings[0]));};var htmlTag={html:html,htmlLiteral:htmlLiteral};const PolymerElement=ElementMixin(HTMLElement);var polymerElement={version:version,PolymerElement:PolymerElement,html:html};const bundledImportMeta$1={...import.meta,url:new URL('../../define-element.js',import.meta.url).href};function UncamelCase(name){return name// insert a hyphen between lower & upper
...
.replace(/ /g,'-')// lowercase
.toLowerCase();}function functionName(func){return typeof func==='function'?func.toString().replace(/^[\S\s]*?function\s*/,"").replace(/[\s\(\/][\S\s]+$/,""):undefined;}var _Define=function(id){return customElements.get(id);};if(!window.Define){Object.defineProperty(window,'Define',{get:function(){return _Define;},set:function(proto){/*
      Patterns:
        a) template id {}
        b) template id {is}
        ...
        f) class Is {template}
        g) {is,template}
      */var id;var classId;var obj;var name=proto.hasOwnProperty('name')?proto.name:functionName(proto);var current;// currentScript
var template=null;var previous;// previousSibling template
var cousin;// dom5.serialize output support
current=!window.HTMLImports||HTMLImports.useNative?document.currentScript:document._currentScript||document.currentScript;if(!current){let url='';//import.meta.url;
let scripts=document.querySelectorAll('script[type=module]');scripts.forEach(script=>{let src=new URL(script.src,location.href);if(url===src){current=script;}});if(!current){current=scripts[0];}}var _tmpNode=current;var ownerDocument=current?current.ownerDocument:document;var baseURI=ownerDocument.baseURI;if(current&&current.ownerDocument&&current.ownerDocument.nodeType===current.ownerDocument.DOCUMENT_NODE){while(_tmpNode&&_tmpNode.tagName!=='LINK'&&_tmpNode.nodeType!==_tmpNode.DOCUMENT_FRAGMENT_NODE&&_tmpNode.nodeType!==_tmpNode.DOCUMENT_NODE){_tmpNode=_tmpNode.parentNode;}if(_tmpNode&&(_tmpNode.nodeType===_tmpNode.DOCUMENT_FRAGMENT_NODE||_tmpNode.nodeType===_tmpNode.DOCUMENT_NODE)){ownerDocument=_tmpNode;baseURI=ownerDocument.baseURI;}else if(_tmpNode&&_tmpNode.import===_tmpNode){ownerDocument=_tmpNode;baseURI=ownerDocument.href;// link node
}}previous=current&&current.previousSibling;while(previous&&!previous.tagName){previous=previous.previousSibling;}if(previous&&previous.tagName!=='template'.toUpperCase()){previous=null;}if(!previous){// search for cousin template
if(current&&current.parentNode.tagName==='body'.toUpperCase()){previous=current.parentNode.previousSibling;while(previous&&!previous.tagName){previous=previous.previousSibling;}if(previous&&previous.tagName.toLowerCase()==='head'){for(var i=0;i<previous.childNodes.length;i++){if(previous.childNodes[i].tagName==='template'.toUpperCase()){cousin=previous.childNodes[i];break;}}}}if(cousin){previous=cousin;}else{previous=null;}}if(!proto.is&&(!name||name==='class'||name==='Define')){if(previous){id=previous.id;if(id){// Pattern a)
template=previous;proto.is=id;}}}else{if(proto.is){id=proto.is;}else if(typeof proto==='function'&&name){// ES6 class
id=UncamelCase(name);classId=name;}var isNoIdPrevious=previous&&!previous.id&&id;if(isNoIdPrevious){previous.id=id;// temporarily supply id
}var rawTemplate=proto._rawTemplate||proto.template;if(isNoIdPrevious){previous.removeAttribute('id');}if(!template&&rawTemplate&&!name){// Pattern g)
template=rawTemplate;}if(!rawTemplate&&!template){// Pattern b), c)
template=ownerDocument.querySelector('template[id='+id+']')||document.querySelector('template[id='+id+']');}if(!rawTemplate&&!template&&previous&&!previous.id){// Pattern d)
template=previous;template.id=id;}else{// Pattern e)
}}if(!id){throw'Custom element name is not defined';}// register dom-module
if(template){var domModule=document.createElement('dom-module');var assetpath=typeof URL==='function'&&URL.name==='URL'?new URL(baseURI||document.baseURI).pathname:(uri=>{let a=document.createElement('a');a.href=uri;return('/'+a.pathname).replace(/^\/\//,'/');})(baseURI);domModule.appendChild(template);domModule.setAttribute('assetpath',template.hasAttribute('basepath')?template.getAttribute('basepath'):template.hasAttribute('assetpath')?template.getAttribute('assetpath'):assetpath);domModule.register(id);proto._template=template;proto.finalize();}// register Custom Element
classId=classId||id.split('-').map(function(word){return word[0].toUpperCase()+word.substr(1);}).join('');if(customElements.get(id)){console.warn('Discarding duplicate definition of custom element '+id);}else{customElements.define(id,proto);_Define[classId]=customElements.get(id);}return customElements.get(id);}});}/**
  @license
  Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
  This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
  The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
                               ...
                               */DomApi.prototype.replaceChild;/**
                                ...
                                */DomApi.prototype.removeAttribute;/**
                                 ...
                                 */DomApi.prototype.querySelectorAll;/** @type {?Node} */DomApi.prototype.parentNode;/** @type {?Node} */DomApi.prototype.firstChild;/** @type {?Node} */DomApi.prototype.lastChild;/** @type {?Node} */DomApi.prototype.nextSibling;/** @type {?Node} */DomApi.prototype.previousSibling;/** @type {?HTMLElement} */DomApi.prototype.firstElementChild;/** @type {?HTMLElement} */DomApi.prototype.lastElementChild;/** @type {?HTMLElement} */DomApi.prototype.nextElementSibling;/** @type {?HTMLElement} */DomApi.prototype.previousElementSibling;/** @type {!Array<!Node>} */DomApi.prototype.childNodes;/** @type {!Array<!HTMLElement>} */DomApi.prototype.children;/** @type {?DOMTokenList} */DomApi.prototype.classList;/** @type {string} */DomApi.prototype.textContent;/** @type {string} */DomApi.prototype.innerHTML;forwardMethods(DomApi.prototype,['cloneNode','appendChild','insertBefore','removeChild','replaceChild','setAttribute','removeAttribute','querySelector','querySelectorAll']);forwardReadOnlyProperties(DomApi.prototype,['parentNode','firstChild','lastChild','nextSibling','previousSibling','firstElementChild','lastElementChild','nextElementSibling','previousElementSibling','childNodes','children','classList']);forwardProperties(DomApi.prototype,['textContent','innerHTML']);/**
                                                                    ...
                                                                    */const dom=function(obj){obj=obj||document;if(!obj.__domApi){let helper;if(obj instanceof Event){helper=new EventApi(obj);}else{helper=new DomApi(obj);}obj.__domApi=helper;}return obj.__domApi;};var polymer_dom={matchesSelector:matchesSelector,DomApi:DomApi,EventApi:EventApi,dom:dom,flush:flush$1,addDebouncer:enqueueDebouncer};const bundledImportMeta$2={...import.meta,url:new URL('../../node_modules/%40polymer/polymer/lib/legacy/legacy-element-mixin.js',import.meta.url).href};let styleInterface=window.ShadyCSS;/**
...
this.errorMessage=ex.message;Base._error('Could not save to localStorage. Incognito mode may be blocking this action',ex);}}/**
     ...
     */ /**
         ...
         * @param {{externalChange:boolean}} detail -
         *     externalChange: true if change occured in different window.
         */});const bundledImportMeta$5={...import.meta,url:new URL('../../node_modules/i18n-behavior/i18n-preference.js',import.meta.url).href};const $_documentContainer$1=document.createElement('template');$_documentContainer$1.innerHTML=`<template id="i18n-preference">
...
var template=$_documentContainer$1.content.querySelector('template#i18n-preference');var domModule=document.createElement('dom-module');//var currentScript = (!window.HTMLImports || HTMLImports.hasNative || HTMLImports.useNative) ? document.currentScript : (document._currentScript || document.currentScript);
...
domModule.appendChild(template);domModule.register('i18n-preference');Polymer$1({importMeta:bundledImportMeta$5,is:'i18n-preference',properties:{/**
...
this.$.storage.value=undefined;}}},/**
       ...
       */_onPersistChange:function(value){if(this.isReady){if(value){if(this.$.storage.value!==html.getAttribute('lang')){// save to the storage
...
this.$.storage.value=undefined;}}},/**
       ...
       */_onStorageValueChange:function(e){var value=e.detail.value;if(this.isReady){if(this.persist){if(value){if(value!==html.getAttribute('lang')){// save to the lang
...
if(!this._htmlLangMutationObserver){this._htmlLangMutationObserverCallbackBindThis=this._htmlLangMutationObserverCallback.bind(this);this._htmlLangMutationObserver=new MutationObserver(this._htmlLangMutationObserverCallbackBindThis);}this._htmlLangMutationObserver.observe(html,{attributes:true});},/**
       ...
       */_disconnect:function(){if(this._htmlLangMutationObserver){this._htmlLangMutationObserver.disconnect();}}});};if(!window.HTMLImports||HTMLImports.hasNative||HTMLImports.useNative){registerI18nPreference();}else{HTMLImports.whenReady(registerI18nPreference);}})(document);const bundledImportMeta$6={...import.meta,url:new URL('../../node_modules/i18n-behavior/i18n-attr-repo.js',import.meta.url).href};const $_documentContainer$2=document.createElement('template');$_documentContainer$2.innerHTML=`<template id="i18n-attr-repo">
...
var template=$_documentContainer$2.content.querySelector('template#i18n-attr-repo');var domModule$1=document.createElement('dom-module');domModule$1.appendChild(template);domModule$1.register('i18n-attr-repo');window.BehaviorsStore=window.BehaviorsStore||{};// Polymer function for iron-component-page documentation
var Polymer$2=function(proto){BehaviorsStore._I18nAttrRepo=proto;BehaviorsStore._I18nAttrRepo._created();return Polymer$1(proto);};/*
   ...
   */Polymer$2({importMeta:bundledImportMeta$6,is:'i18n-attr-repo',created:function(){this.data=sharedData;var customAttributes=dom(this).querySelector('template#custom');// traverse custom attributes repository
...
return false;},/**
   ...
   */_traverseTemplateTree:function(node){var name;if(node.nodeType===node.ELEMENT_NODE){name=node.nodeName.toLowerCase();Array.prototype.forEach.call(node.attributes,function(attribute){this.data[name]=this.data[name]||{};this.setLocalizableAttribute(name,attribute.name,attribute.value);},this);}if(node.childNodes.length>0){for(var i=0;i<node.childNodes.length;i++){this._traverseTemplateTree(node.childNodes[i]);}}}});/*!
...
ref=reference[index];}clone[key]=ref||recursiveCopy(value,customizer,result,visited,reference);}return clone;}exports['default']=deepcopy;module.exports=exports['default'];/***/}]/******/));}();var deepcopy$1={default:deepcopy};//$_documentContainer.innerHTML = `<i18n-attr-repo></i18n-attr-repo>`;
//document.head.appendChild($_documentContainer.content);
/* jshint -W100 */(function(document){'use strict';var html$$1=document.querySelector('html');if(window.ShadowDOMPolyfill){// Fix #38. Add reflectToAttribute effect on html.lang property
// for supplementing Shadow DOM MutationObserver polyfill
Object.defineProperty(html$$1,'lang',{get:function(){return this.getAttribute('lang');},set:function(value){this.setAttribute('lang',value);}});}// Safari 7 predefines non-configurable standard properties
...
var extraWhiteSpaceNode=!isStandardPropertyConfigurable;if(ElementMixin){isStandardPropertyConfigurable=false;}// app global bundle storage
var bundles={'':{}};// with an empty default bundle
// app global default language
var defaultLang=html$$1.hasAttribute('lang')?html$$1.getAttribute('lang'):'';// shared fetching instances for bundles
...
var localesPath=html$$1.hasAttribute('locales-path')?html$$1.getAttribute('locales-path'):'locales';// Support ShadowDOM V1 on Polymer 2.x
var paramAttribute=ElementMixin?'slot':'param';var attributesRepository=document.createElement('i18n-attr-repo');// ((!window.HTMLImports || HTMLImports.hasNative || HTMLImports.useNative) ? document.currentScript : (document._currentScript || document.currentScript)).ownerDocument.querySelector('i18n-attr-repo');
...
var debuglog=html$$1.hasAttribute('debug')?function(arg){console.log(arg);}:function(){};window.BehaviorsStore=window.BehaviorsStore||{};/**
...
event.target.removeEventListener(event.type,this._handleBundleFetchedBindThis);if(this._fetchStatus.ajaxLang===detail.lang){this._fetchStatus.fetchingInstance=null;this._fetchBundle(this._fetchStatus.ajaxLang);}},/**
     ...
     */_observeHtmlLangChanged:function(value){if(value){this._htmlLangObserver=this._htmlLangObserver||new MutationObserver(this._handleHtmlLangChange.bind(this));this._htmlLangObserver.observe(html$$1,{attributes:true});}else{if(this._htmlLangObserver){this._htmlLangObserver.disconnect();}}},/**
     * MutationObserver callback of `<html lang>` attribute.
     *
     * @param {Array} mutations Array of MutationRecord (https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver).
     */_handleHtmlLangChange:function(mutations){mutations.forEach(function(mutation){switch(mutation.type){case'attributes':if(mutation.attributeName==='lang'){this.lang=html$$1.lang;}break;default:break;}},this);},/**
...
return true;},/**
     ...
     */_traverseAttributes:function(node,path,bundle){var name=node.nodeName.toLowerCase();var id=node.getAttribute?node.getAttribute('text-id')||node.getAttribute('id'):null;var text;var messageId;var attrId;var isLocalizable;var dummy;// pick up element attributes
...
if(ElementMixin){// Avoid ShadyDOM issue for Polymer 2.x (Implicit removal by appendChild to another element introduces inconsistencies)
node.innerHTML='';}templateText=document.createElement('i18n-format');templateText.setAttribute('lang','{{effectiveLang}}');if(ElementMixin){// Avoid ShadyDOM issue for Polymer 2.x (Implicit removal by appendChild to another element introduces inconsistencies)
// insert i18n-format
dom(node).appendChild(templateText);}span=document.createElement('span');// span.innerText does not set an effective value in Firefox
span.textContent=templateTextParams.params.shift();templateText.appendChild(span);Array.prototype.forEach.call(templateTextParams.params,function(param){templateText.appendChild(param);});if(!ElementMixin){// Avoid ShadyDOM issue for Polymer 1.x (Clearance of innerHTML unexpectedly removes textContent of child nodes)
// insert i18n-format
node.innerHTML='';dom(node).appendChild(templateText);}// store the text message
...
dom(parent).insertBefore(templateText,node);dom(parent).removeChild(node);span=document.createElement('span');// span.innerText does not set an effective value in Firefox
...
/**
     ...
     */beforeRegister:function(){if(ElementMixin){return;}if(this.is!=='i18n-dom-bind'){if(!this._templateLocalizable){this._templateLocalizable=this._constructDefaultBundle();}}},/**
...
this.properties=this._properties;delete this._properties;}}if(this.observeHtmlLang){this.lang=html$$1.lang;// TODO: this call is redundant
...
if(typeof this.observeHtmlLang==='undefined'&&!this.hasAttribute('observe-html-lang')){this.observeHtmlLang=true;}if(this.observeHtmlLang){this.lang=html$$1.lang;this._observeHtmlLangChanged(true);}},/**
...
if(!isStandardPropertyConfigurable){var _properties=Object.create(null);for(var p in BehaviorsStore.I18nBehavior.properties){if(p==='lang'){_properties._lang=BehaviorsStore.I18nBehavior.properties.lang;}else{_properties[p]=BehaviorsStore.I18nBehavior.properties[p];}}BehaviorsStore.I18nBehavior.properties=_properties;BehaviorsStore.I18nBehavior.properties._lang.reflectToAttribute=false;BehaviorsStore.I18nBehavior.properties.text.computed='_getBundle(_lang)';BehaviorsStore.I18nBehavior._updateEffectiveLang=function(event){if(!ElementMixin&&dom(event).rootTarget===this||ElementMixin&&event.composedPath()[0]===this){//console.log('lang-updated: _updateEffectiveLang: assigning effectiveLang = ' + this._lang);
this.effectiveLang=this._lang;}};BehaviorsStore.I18nBehavior.hostAttributes={'lang':defaultLang};}if(ElementMixin){// Polymer 2.x
BehaviorsStore._I18nBehavior=BehaviorsStore.I18nBehavior;BehaviorsStore.I18nBehavior=[BehaviorsStore._I18nBehavior];if(!document.currentScript){// Polymer 3.x
...
BehaviorsStore.I18nBehavior.push(MutableDataBehavior);}}Object.defineProperty(BehaviorsStore.I18nBehavior,'0',{get:function(){var current=!window.HTMLImports||HTMLImports.hasNative||HTMLImports.useNative?document.currentScript:document._currentScript||document.currentScript;var ownerDocument=document;//current.ownerDocument;
...
/**
     ...
     */var i18nBehaviorDomBind={};Base.extend(i18nBehaviorDomBind,BehaviorsStore.I18nBehavior);var i18nDomBind={};var domBind=document.createElement('template','dom-bind');var domBindProto=Object.getPrototypeOf(domBind);if(typeof domBindProto.render!=='function'){domBindProto=domBind.__proto__;// fallback for IE10
}Base.extend(i18nDomBind,domBindProto);i18nDomBind.is='i18n-dom-bind';if(!navigator.language&&navigator.browserLanguage){// Detect IE10
...
});});}(params.setup?teardown:suiteTeardown)(function(){restoreFixture(params.fixture);});});});};const bundledImportMeta$7={...import.meta,url:new URL('./simple-text-element/simple-text-element.js',import.meta.url).href};const $_documentContainer$3=document.createElement('template');$_documentContainer$3.innerHTML=`<template id="simple-text-element">
...
`),is:'simple-text-element',behaviors:[BehaviorsStore.I18nBehavior],listeners:{'lang-updated':'_langUpdated'},ready:function(){//this.observeHtmlLang = false;
},attached:function(){},_langUpdated:function(e){console.log(this.is,'lang-updated',e.detail);if(dom(e).rootTarget===this){this.model=deepcopy(this.text.model);}}});}break;}const bundledImportMeta$8={...import.meta,url:new URL('./commented-simple-text-element/commented-simple-text-element.js',import.meta.url).href};const $_documentContainer$4=document.createElement('template');$_documentContainer$4.innerHTML=`<template id="commented-simple-text-element"><!-- comment -->
...
`),is:'commented-simple-text-element',behaviors:[BehaviorsStore.I18nBehavior],listeners:{'lang-updated':'_langUpdated'},ready:function(){//this.observeHtmlLang = false;
},attached:function(){},_langUpdated:function(e){console.log(this.is,'lang-updated',e.detail);if(dom(e).rootTarget===this){this.model=deepcopy(this.text.model);}}});}break;}const bundledImportMeta$9={...import.meta,url:new URL('./plural-gender-element/plural-gender-element.js',import.meta.url).href};const $_documentContainer$5=document.createElement('template');$_documentContainer$5.innerHTML=`<template id="plural-gender-element">
    <p>
...
`),is:'plural-gender-element',behaviors:[BehaviorsStore.I18nBehavior],properties:{sender:{type:Object},recipients:{type:Array}},listeners:{'lang-updated':'_langUpdated'},_langUpdated:function(e){console.log('plural-gender-element lang-updated lang = '+this.lang+' effectiveLang = '+this.effectiveLang);if(e.composedPath()[0]===this){this.model=deepcopy(this.text.model);}if(this.renderedEffectiveLang===this.effectiveLang||this.renderedEffectiveLang===''&&this.effectiveLang==='en'){this.fire('local-dom-ready');}},_rendered:function(){console.log('plural-gender-element rendered lang = '+this.lang+' effectiveLang = '+this.effectiveLang);if(this.lang===this.effectiveLang){this.fire('local-dom-ready');}else{this.renderedEffectiveLang=this.effectiveLang;}}});}break;}const bundledImportMeta$a={...import.meta,url:new URL('./simple-attribute-element/text-attribute-element.js',import.meta.url).href};const $_documentContainer$6=document.createElement('template');$_documentContainer$6.innerHTML=`<i18n-attr-repo>
  <template id="custom">
...
`);}static get is(){return'text-attribute-element';}static get properties(){return{customTextAttr1:{type:String,reflectToAttribute:true},customTextAttr2:{type:String,reflectToAttribute:true},customTextAttr3:{type:String,reflectToAttribute:true},outOfScopeAttr:{type:String,reflectToAttribute:true},i18nTarget:{type:String},i18nTarget2:{type:String}};}ready(){this.addEventListener('lang-updated',this._langUpdated);super.ready();}_langUpdated(){this.model=deepcopy(this.text.model);}}customElements.define(TextAttributeElement.is,TextAttributeElement);}break;case'base-element':{class TextAttributeElement extends BaseElements.I18nElement{static get importMeta(){return bundledImportMeta$a;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <span id="attr1">{{customTextAttr1}}</span>
    <span id="attr2">{{customTextAttr2}}</span>
    <span id="attr3">{{customTextAttr3}}</span>
    <span id="attr4">{{outOfScopeAttr}}</span>
...
`);}static get is(){return'text-attribute-element';}static get properties(){return{customTextAttr1:{type:String,reflectToAttribute:true},customTextAttr2:{type:String,reflectToAttribute:true},customTextAttr3:{type:String,reflectToAttribute:true},outOfScopeAttr:{type:String,reflectToAttribute:true},i18nTarget:{type:String},i18nTarget2:{type:String}};}ready(){this.addEventListener('lang-updated',this._langUpdated);super.ready();}_langUpdated(){this.model=deepcopy(this.text.model);}}customElements.define(TextAttributeElement.is,TextAttributeElement);}break;case'thin':{Define=class TextAttributeElement extends BaseElements.I18nElement{static get importMeta(){return bundledImportMeta$a;}static get properties(){return{customTextAttr1:{type:String,reflectToAttribute:true},customTextAttr2:{type:String,reflectToAttribute:true},customTextAttr3:{type:String,reflectToAttribute:true},outOfScopeAttr:{type:String,reflectToAttribute:true},i18nTarget:{type:String},i18nTarget2:{type:String}};}ready(){this.addEventListener('lang-updated',this._langUpdated);super.ready();}_langUpdated(){this.model=deepcopy(this.text.model);}};}break;case'legacy':{Polymer$1({importMeta:bundledImportMeta$a,_template:(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <span id="attr1">{{customTextAttr1}}</span>
    <span id="attr2">{{customTextAttr2}}</span>
    <span id="attr3">{{customTextAttr3}}</span>
    <span id="attr4">{{outOfScopeAttr}}</span>
...
`),is:'text-attribute-element',behaviors:[BehaviorsStore.I18nBehavior],properties:{customTextAttr1:{type:String,reflectToAttribute:true},customTextAttr2:{type:String,reflectToAttribute:true},customTextAttr3:{type:String,reflectToAttribute:true},outOfScopeAttr:{type:String,reflectToAttribute:true},i18nTarget:{type:String},i18nTarget2:{type:String}},listeners:{'lang-updated':'_langUpdated'},ready:function(){//this.observeHtmlLang = false;
},attached:function(){},_langUpdated:function(){this.model=deepcopy(this.text.model);}});}break;}const bundledImportMeta$b={...import.meta,url:new URL('./simple-attribute-element/simple-attribute-element.js',import.meta.url).href};const $_documentContainer$7=document.createElement('template');$_documentContainer$7.innerHTML=`<template id="simple-attribute-element">
...
`);}static get is(){return'simple-attribute-element';}ready(){this.addEventListener('lang-updated',this._langUpdated);super.ready();}connectedCallback(){super.connectedCallback();this._intervalId=window.setInterval(this._checkChartStatus.bind(this),1000);setTimeout(function(){this.isPieChartRendered=true;this.isColumnChartRendered=true;window.clearInterval(this._intervalId);this.fire('local-dom-ready');// TODO: this is an ad-hoc solution
}.bind(this),200);}disconnectedCallback(){super.disconnectedCallback();}_checkChartStatus(){var pieChartInnerHtml=this.$['pie-chart'].$.chartdiv.innerHTML;var columnChartInnerHtml=this.$['column-chart'].$.chartdiv.innerHTML;var notLoaded=false;if(pieChartInnerHtml==='Undefined chart type'){this.isPieChartRendered=true;// google api to blame; this is not responsible
}if(columnChartInnerHtml==='Undefined chart type'){this.isColumnChartRendered=true;// google api to blame; this is not responsible
}if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}_langUpdated(e){console.log('_langUpdated');if(e.composedPath()[0]===this){this.model=deepcopy(this.text.model);}}_pieChartRendered(){this.isPieChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}_columnChartRendered(){this.isColumnChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}}customElements.define(SimpleAttributeElement.is,SimpleAttributeElement);}break;case'base-element':{class SimpleAttributeElement extends BaseElements.I18nElement{static get importMeta(){return bundledImportMeta$b;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <style attr="This is not extracted">
    google-chart {
      width: 300px;
    }
...
}if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}_langUpdated(e){if(e.composedPath()[0]===this){this.model=deepcopy(this.text.model);}}_pieChartRendered(){this.isPieChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}_columnChartRendered(){this.isColumnChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}}customElements.define(SimpleAttributeElement.is,SimpleAttributeElement);}break;case'thin':{Define=class SimpleAttributeElement extends BaseElements.I18nElement{static get importMeta(){return bundledImportMeta$b;}ready(){this.addEventListener('lang-updated',this._langUpdated);super.ready();}connectedCallback(){super.connectedCallback();this._intervalId=window.setInterval(this._checkChartStatus.bind(this),1000);setTimeout(function(){this.isPieChartRendered=true;this.isColumnChartRendered=true;window.clearInterval(this._intervalId);this.fire('local-dom-ready');// TODO: this is an ad-hoc solution
}.bind(this),200);}disconnectedCallback(){super.disconnectedCallback();}_checkChartStatus(){var pieChartInnerHtml=this.$['pie-chart'].$.chartdiv.innerHTML;var columnChartInnerHtml=this.$['column-chart'].$.chartdiv.innerHTML;var notLoaded=false;if(pieChartInnerHtml==='Undefined chart type'){this.isPieChartRendered=true;// google api to blame; this is not responsible
}if(columnChartInnerHtml==='Undefined chart type'){this.isColumnChartRendered=true;// google api to blame; this is not responsible
}if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}_langUpdated(e){if(e.composedPath()[0]===this){this.model=deepcopy(this.text.model);}}_pieChartRendered(){this.isPieChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}_columnChartRendered(){this.isColumnChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}};}break;case'legacy':{Polymer$1({importMeta:bundledImportMeta$b,_template:(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <style attr="This is not extracted">
    google-chart {
      width: 300px;
    }
...
`),is:'simple-attribute-element',behaviors:[BehaviorsStore.I18nBehavior],listeners:{'lang-updated':'_langUpdated','pie-chart.google-chart-render':'_pieChartRendered','column-chart.google-chart-render':'_columnChartRendered'},ready:function(){//this.observeHtmlLang = false;
},attached:function(){this._intervalId=window.setInterval(this._checkChartStatus.bind(this),1000);setTimeout(function(){this.isPieChartRendered=true;this.isColumnChartRendered=true;window.clearInterval(this._intervalId);this.fire('local-dom-ready');// TODO: this is an ad-hoc solution
}.bind(this),200);},_checkChartStatus:function(){var pieChartInnerHtml=this.$['pie-chart'].$.chartdiv.innerHTML;var columnChartInnerHtml=this.$['column-chart'].$.chartdiv.innerHTML;var notLoaded=false;if(pieChartInnerHtml==='Undefined chart type'){this.isPieChartRendered=true;// google api to blame; this is not responsible
}if(columnChartInnerHtml==='Undefined chart type'){this.isColumnChartRendered=true;// google api to blame; this is not responsible
}if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}},_langUpdated:function(e){if(dom(e).rootTarget===this){this.model=deepcopy(this.text.model);}},_pieChartRendered:function(){this.isPieChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}},_columnChartRendered:function(){this.isColumnChartRendered=true;if(this.isPieChartRendered&&this.isColumnChartRendered){window.clearInterval(this._intervalId);this.fire('local-dom-ready');}}});}break;}const bundledImportMeta$c={...import.meta,url:new URL('./fallback-text-element/fallback-text-element.js',import.meta.url).href};const $_documentContainer$8=document.createElement('template');$_documentContainer$8.innerHTML=`<template id="fallback-text-element">
    outermost text at the beginning 
    <div><div></div></div><!-- nested empty div -->
    <span id="whitespace"> &nbsp; </span>
    <h1>outermost header 1</h1>
...
`),is:'fallback-text-element',behaviors:[BehaviorsStore.I18nBehavior],listeners:{'lang-updated':'_langUpdated'},ready:function(){//this.observeHtmlLang = false;
},attached:function(){},_langUpdated:function(e){console.log(this.is,'lang-updated',e.detail);if(dom(e).rootTarget===this){this.model=deepcopy(this.text.model);}}});}break;}const bundledImportMeta$d={...import.meta,url:new URL('./compound-binding-element/compound-binding-element.js',import.meta.url).href};const $_documentContainer$9=document.createElement('template');$_documentContainer$9.innerHTML=`<template id="compound-binding-element">
...
`),is:'compound-binding-element',behaviors:[BehaviorsStore.I18nBehavior],properties:{param1:{type:String,value:'parameter 1'},param2:{type:String,value:'parameter 2'}},listeners:{'lang-updated':'_langUpdated'},ready:function(){//this.observeHtmlLang = false;
},attached:function(){},_langUpdated:function(e){console.log(this.is,'lang-updated',e.detail);if(dom(e).rootTarget===this){this.model=deepcopy(this.text.model);}}});}break;}const bundledImportMeta$e={...import.meta,url:new URL('./simple-text-id-element/simple-text-id-element.js',import.meta.url).href};const $_documentContainer$a=document.createElement('template');$_documentContainer$a.innerHTML=`<template id="simple-text-id-element">
    outermost text at the beginning 
    <div><div></div></div><!-- nested empty div -->
