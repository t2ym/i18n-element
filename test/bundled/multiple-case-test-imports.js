...
});});}(params.setup?teardown:suiteTeardown)(function(){restoreFixture(params.fixture);});});});};const bundledImportMeta$7={...import.meta,url:new URL('./multiple-case/item-element.js',import.meta.url).href};const $_documentContainer$3=document.createElement('template');$_documentContainer$3.innerHTML=`<template id="item-element">
    <span id="label">A</span>
  </template>`;document.head.appendChild($_documentContainer$3.content);switch(syntax){default:case'mixin':{class ItemElement extends Mixins.Localizable(LegacyElementMixin(HTMLElement)){static get importMeta(){return bundledImportMeta$7;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <span id="label">{{text.label}}</span>
<template id="localizable-text">
<json-data>
{
...
`);}static get is(){return'item-element';}}customElements.define(ItemElement.is,ItemElement);}break;case'base-element':{class ItemElement extends BaseElements.I18nElement{static get importMeta(){return bundledImportMeta$7;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <span id="label">{{text.label}}</span>
<template id="localizable-text">
<json-data>
{
...
`),is:'item-element',behaviors:[BehaviorsStore.I18nBehavior]});}break;}const bundledImportMeta$8={...import.meta,url:new URL('./multiple-case/multiple-element.js',import.meta.url).href};const $_documentContainer$4=document.createElement('template');$_documentContainer$4.innerHTML=`<template id="multiple-element">
    <div id="base">
    ...
    <div id="save"></div>
  </template>`;document.head.appendChild($_documentContainer$4.content);switch(syntax){default:case'mixin':{class MultipleElement extends Mixins.Localizable(LegacyElementMixin(HTMLElement)){static get importMeta(){return bundledImportMeta$8;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <div id="base">
      <dom-repeat id="items" items="{{getArray(count)}}" on-dom-change="domChanged"><template>
        <span>
          <item-element lang="{{effectiveLang}}" observe-html-lang="{{observeHtmlLang}}"></item-element>
...
`);}static get is(){return'multiple-element';}static get properties(){return{count:{type:Number,value:100}};}ready(){this.addEventListener('lang-updated',this.langUpdated);super.ready();}getArray(count){var a=[];for(var i=0;i<count;i++){a.push(i);}return a;}domChanged(e){var nodes=dom(this.root).querySelectorAll('item-element');if(this.lang==='en'&&this.effectiveLang===''){this.effectiveLang='en';}//console.log('multiple-element: dom-change count = ' + nodes.length + ' lang = ' + this.lang + ' effectiveLang = ' + this.effectiveLang);
...
this.$.items.render();}return false;}}customElements.define(MultipleElement.is,MultipleElement);}break;case'base-element':{class MultipleElement extends BaseElements.I18nElement{static get importMeta(){return bundledImportMeta$8;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <div id="base">
      <dom-repeat id="items" items="{{getArray(count)}}" on-dom-change="domChanged"><template>
        <span>
          <item-element lang="{{effectiveLang}}" observe-html-lang="{{observeHtmlLang}}"></item-element>
...
//            ' itemLang[' + this.lang + '] = ' + this.itemLang[this.lang]);
if(this.itemLang[this.lang]===this.count){//console.log('count reached for ' + this.lang);
this.$.items.render();}return false;}};}break;case'legacy':{Polymer$1({importMeta:bundledImportMeta$8,_template:(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <div id="base">
      <dom-repeat id="items" items="{{getArray(count)}}" on-dom-change="domChanged"><template>
        <span>
          <item-element lang="{{effectiveLang}}" observe-html-lang="{{observeHtmlLang}}"></item-element>
