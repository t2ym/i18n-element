...
$_documentContainer.innerHTML = `<template id="null-template-default-lang-element" lang="" basepath="template-default-lang/" localizable-text="embedded">{{text.text}}<div><div></div></div><!-- nested empty div -->
...
</template><dom-module id="null-template-default-lang-element" legacy="">
  <template lang="" localizable-text="embedded">{{text.text}}<div><div></div></div><!-- nested empty div -->
    <span id="whitespace"> &nbsp; </span>
    <h1>{{text.h1_3}}</h1>{{text.text_4}}<span>{{text.span_5}}</span>
    <span>{{text.span_6}}</span>
    <span id="label-1">{{text.label-1}}</span>
