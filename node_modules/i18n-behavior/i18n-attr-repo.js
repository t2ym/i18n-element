...
window.BehaviorsStore = window.BehaviorsStore || {}; // Polymer function for iron-component-page documentation
...
Polymer({
  ...
  created: function () {
    ...
    var customAttributes = this.querySelector('template#custom'); // traverse custom attributes repository
  ...
  isLocalizableAttribute: function (element, attr) {
    ...
    if (this.data['any-elements'] && this.data['any-elements'][attr]) {
      return this.data['any-elements'][attr];
    } else if (this.data[tagName]) {
      return this.data[tagName]['any-attributes'] || this._getType(element, this.data[tagName][attr]);
    } else {
      return false;
    }
  },
  ...
  setLocalizableAttribute: function (element, attr, value) {
    ...
    if (typeof cursor[prev] === 'object' && cursor[prev] && Object.keys(cursor[prev]).length) {
      cursor = cursor[prev];
      prev = '';
    }
