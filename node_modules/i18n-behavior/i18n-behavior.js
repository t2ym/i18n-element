...
import deepcopy from "../deepcopy/dist/deepcopy.js"; // html tag of the document

var html = document.querySelector('html'); // app global bundle storage

var bundles = {
  '': {}
}; // with an empty default bundle
...
var startUrl = function () {
  ...
  if (document.querySelector('meta[name=app-root]') && document.querySelector('meta[name=app-root]').getAttribute('content')) {
  ...
  } else if (document.querySelector('link[rel=manifest]') && document.querySelector('link[rel=manifest]').getAttribute('href') && document.querySelector('link[rel=manifest]').getAttribute('href').match(/^\//)) {
...
var localesPath = html.hasAttribute('locales-path') ? html.getAttribute('locales-path') : 'locales'; // Support ShadowDOM V1

var paramAttribute = 'slot';
var attributesRepository = document.createElement('i18n-attr-repo'); // set up userPreference

var userPreference = document.querySelector('i18n-preference');

if (!userPreference) {
  userPreference = document.createElement('i18n-preference'); // append to body
...
var debuglog = html.hasAttribute('debug') ? function (arg) {
  console.log(arg);
} : function () {};
...
let I18nBehavior = {
  ...
  _getBundle: function (lang) {
    ...
    if (!resolved) {
      ...
      } else if (defaultLang && defaultLang.length > 0 && bundles[defaultLang] && bundles[defaultLang][id]) {
        // app default language for now
        resolved = bundles[defaultLang][id];
      } else if (this.templateDefaultLang && this.templateDefaultLang.length > 0 && bundles[this.templateDefaultLang] && bundles[this.templateDefaultLang][id]) {
  ...
  _enumerateFallbackLanguages: function (lang) {
    ...
    if (!lang || lang.length === 0) {
      ...
      parts = lang.split(/[-_]/); // normalize ISO-639-1 language codes

      if (parts.length > 0 && parts[0].match(/^[A-Za-z]{2,3}$/)) {
      ...
      if (parts.length >= 2 && parts[1].match(/^[A-Za-z]{3}$/) && !parts[1].match(/^[Cc][Hh][SsTt]$/)) {
        // exclude CHS,CHT
      ...
      if (parts.length >= isExtLangCode + 2 && (match = parts[isExtLangCode + 1].match(/^([A-Za-z])([A-Za-z]{3})$/))) {
      ...
      if (parts.length >= isExtLangCode + isScriptCode + 2 && (match = parts[isExtLangCode + isScriptCode + 1].match(/^[A-Za-z0-9]{2,3}$/))) {
      ...
      while (parts.length > 0) {
        ...
        if (isScriptCode && isCountryCode && parts.length == isExtLangCode + isScriptCode + 2) {
        ...
        if (isExtLangCode && isCountryCode && parts.length == isExtLangCode + isScriptCode + 2) {
        ...
        if (isExtLangCode && isScriptCode && parts.length == isExtLangCode + isScriptCode + 1) {
          // ext lang code can be omitted to default
        ...
        if (!isScriptCode && !isExtLangCode && isCountryCode && parts.length == 2) {
      ...
      switch (mutation.type) {
        case 'attributes':
          if (mutation.attributeName === 'lang') {
            ...
            if (!(typeof mutation.oldValue === 'object' && !mutation.oldValue) && mutation.oldValue !== this.lang) {
  ...
  _langChanged: function (lang, oldLang) {
    ...
    if (lang !== oldLang && bundles[oldLang] && bundles[oldLang][id]) {
  ...
  _fetchBundle: function (lang) {
    ...
    if (bundles[lang][id]) {
      ...
      if (this._fetchStatus.targetLang === lang) {
        ...
        var nextFallbackLanguage = this._fetchStatus.fallbackLanguageList.shift(); // bundle is available; no need to fetch
        ...
        if (nextFallbackLanguage) {
          ...
          this._constructBundle(this._fetchStatus.targetLang); // reset error status
      ...
      if (this._fetchStatus.fetchingInstance !== this) {
        ...
        this._forwardLangEventBindThis = this._forwardLangEventBindThis || this._forwardLangEvent.bind(this);

        this._fetchStatus.fetchingInstance.addEventListener('lang-updated', this._forwardLangEventBindThis);
      }
      ...
      this._handleBundleFetchedBindThis = this._handleBundleFetchedBindThis || this._handleBundleFetched.bind(this);
      bundleFetchingInstances[lang].addEventListener('bundle-fetched', this._handleBundleFetchedBindThis); //console.log(this.is + ' addEventListener bundle-fetched');
    } else {
      // proceed to fetch
      this._fetchStatus.fetchingInstance = this;
      ...
      var importBaseURI = this.constructor.importMeta ? this.constructor.importMeta.url : location.href;
      ...
      try {
        ...
        if (skipFetching) {
          this._handleError({
            detail: {
              error: 'skip fetching for I18nController'
            }
          });
        } else {
  ...
  _handleResponse: function (event) {
    ...
    if (this._fetchStatus.ajax.url.indexOf('/' + localesPath + '/bundle.') >= 0) {
      ...
      this._deepMap(bundles[this._fetchStatus.ajaxLang], event.detail.response, function (text) {
        return text;
      });

      bundles[this._fetchStatus.ajaxLang].bundle = true;
      bundleFetchingInstances[this._fetchStatus.ajaxLang] = null; //console.log('bundle-fetched ' + this.is + ' ' + this._fetchStatus.ajaxLang);
    ...
    if (this._fetchStatus.lastResponse) {
      var nextFallbackLanguage = this._fetchStatus.fallbackLanguageList.shift(); // store the raw response


      this._fetchStatus.rawResponses[this._fetchStatus.ajaxLang] = this._fetchStatus.lastResponse;
      this._fetchStatus.fetchingInstance = null;
  ...
  _handleError: function (event) {
    ...
    if (this._fetchStatus.ajax.url.indexOf('/' + localesPath + '/bundle.') >= 0) {
      bundles[this._fetchStatus.ajaxLang] = bundles[this._fetchStatus.ajaxLang] || {};
      bundles[this._fetchStatus.ajaxLang].bundle = true;
      bundleFetchingInstances[this._fetchStatus.ajaxLang] = null; // falls back to its element-specific bundle
    ...
    if (this._fetchStatus.ajaxLang === this._fetchStatus.targetLang) {
      if (nextFallbackLanguage) {
        ...
        this._fetchStatus.error = event.detail.error; //console.log(this._fetchStatus.error);
  ...
  _handleBundleFetched: function (event) {
    var detail = event.detail; //console.log('_handleBundleFetched ' + this.is + ' ' + detail.lang);
  ...
  _observeHtmlLangChanged: function (value) {
    if (value) {
      ...
      this._htmlLangObserver.observe(html, {
        attributes: true
      });
    } else {
  ...
  _constructBundle: function (lang) {
    ...
    for (i = 0; i < fallbackLanguageList.length; i++) {
      if (bundles[fallbackLanguageList[i]] && bundles[fallbackLanguageList[i]][id]) {
        break;
      }
    }
  ...
  _constructDefaultBundle: function (_template, _id) {
    ...
    bundles[templateDefaultLang][id] = bundle; //console.log('text = ');
  ...
  _traverseAttributes: function (node, path, bundle) {
    ...
    var isLocalizable;
    var dummy;
    var renamedAttributes = []; // pick up element attributes
    ...
    Array.prototype.forEach.call(node.attributes, function (attribute) {
      ...
      switch (attribute.name) {
        ...
        case 'is':
        case 'lang':
        case 'class': // verification required before removing these attributes
        ...
        default:
          ...
          if (text.length === 0) {// skip empty value attribute
          } else if (text.match(/^{{[^{}]*}}$/) || text.match(/^\[\[[^\[\]]*\]\]$/)) {// skip annotation attribute
          } else if (text.replace(/\n/g, ' ').match(/^{.*}|\[.*\]$/g) && !text.match(/^{{[^{}]*}}|\[\[[^\[\]]*\]\]/) && !text.match(/{{[^{}]*}}|\[\[[^\[\]]*\]\]$/)) {
            // generate message id
  ...
  _traverseTemplateTree: function (node, path, bundle, index) {
    ...
    path.push(id ? '#' + id : name + (index > 0 ? '_' + index : '')); //console.log(path.join(':'));
    ...
    switch (node.nodeType) {
        ...
        switch (name) {
          ...
          case 'i18n-format':
            ...
            text = Array.prototype.filter.call(node.childNodes, function (child) {
              ...
              var parsedValue = value.match(/^({{)(.*)(}})$/) || value.match(/^(\[\[)(.*)(\]\])$/);

              if (n === 0) {
                // template element
                if (param.tagName.toLowerCase() === 'json-data') {
                  if (parsedValue) {
                    var parsedValue2 = value.match(/^({{)(serialize\(.*\))(}})$/) || value.match(/^(\[\[)(serialize\(.*\))(\]\])$/);
          ...
          default:
            // element node
            if (name === 'i18n-number' || name === 'i18n-datetime') {
              if (!node.hasAttribute('lang')) {
                node.setAttribute('lang', '{{effectiveLang}}');
              }
            } // pick up element attributes
            ...
            if (node.childElementCount === 0 && !isCompoundAnnotatedNode) {
              if (node.textContent) {
                // use textContent for Firefox compatibility
                text = node.textContent;

                if (text.length === 0 || text.match(/^\s*$/g)) {// skip empty or whitespace node
              ...
              var childStatus = Array.prototype.map.call(node.childNodes, function (child) {
                var result;

                if (child.nodeType === child.ELEMENT_NODE && child.tagName === 'TEMPLATE') {
                  var templateNonCommentChildNodes = Array.prototype.filter.call(child.content.childNodes, function (templateChild) {
                  ...
                  var firstChild = templateNonCommentChildNodes.shift(); // Examples:
                  ...
                  result = {
                    hasText: templateNonCommentChildNodes.length === 0 && firstChild && firstChild.nodeType === firstChild.TEXT_NODE && firstChild.textContent.length > 0 && !firstChild.textContent.match(/^\s*$/g),
                    hasCompoundAnnotatedText: firstChild && firstChild.nodeType === firstChild.TEXT_NODE && this._isCompoundAnnotatedText(firstChild.textContent),
                    hasTextChild: templateNonCommentChildNodes.length === 0 && firstChild && firstChild.nodeType === child.ELEMENT_NODE && firstChild.childElementCount === 0,
                    // including <br>
                    hasCompoundAnnotatedChildNode: firstChild && firstChild.nodeType === firstChild.ELEMENT_NODE && firstChild.childElementCount === 0 && this._isCompoundAnnotatedText(firstChild.textContent),
                    hasGrandChildren: templateNonCommentChildNodes.length > 0 || firstChild && firstChild.nodeType === firstChild.ELEMENT_NODE && Array.prototype.map.call(firstChild.childNodes, function (grandChild) {
                      return grandChild.nodeType !== grandChild.TEXT_NODE;
                    }).reduce(function (prev, current) {
                      return prev || current;
                    }, false) || firstChild && firstChild.nodeType === firstChild.TEXT_NODE && this._isCompoundAnnotatedText(firstChild.textContent)
                  };
                } else {
                  result = {
                    hasText: child.nodeType === child.TEXT_NODE && child.textContent.length > 0 && !child.textContent.match(/^\s*$/g),
                    hasCompoundAnnotatedText: child.nodeType === child.TEXT_NODE && this._isCompoundAnnotatedText(child.textContent),
                    hasTextChild: child.nodeType === child.ELEMENT_NODE && child.childElementCount === 0,
                    // including <br>
                    hasCompoundAnnotatedChildNode: child.nodeType === child.ELEMENT_NODE && child.childElementCount === 0 && this._isCompoundAnnotatedText(child.textContent),
                    hasGrandChildren: child.nodeType === child.ELEMENT_NODE && Array.prototype.map.call(child.childNodes, function (grandChild) {
                      return grandChild.nodeType !== grandChild.TEXT_NODE;
                    }).reduce(function (prev, current) {
                      return prev || current;
                    }, false)
              ...
              if ((childStatus.hasText || node.hasAttribute('text-id')) && (childStatus.hasTextChild || childStatus.hasCompoundAnnotatedText) && !childStatus.hasGrandChildren && !childStatus.hasCompoundAnnotatedChildNode) {
                // apply i18n-format
                ...
                */
                n = 0;
                messageId = this._generateMessageId(path, id);
                templateTextParams = Array.prototype.map.call(node.childNodes, function (child) {
                  var firstChild;

                  if (child.nodeType === child.TEXT_NODE && this._hasAnnotatedText(child.textContent)) {
                    return this._compoundAnnotationToSpan(child).map(function (_child) {
                      return {
                        node: _child,
                        templateNode: null,
                        type: _child.nodeType,
                        text: _child.nodeType === _child.TEXT_NODE ? _child.textContent : null,
                        childTextNode: _child.nodeType === _child.ELEMENT_NODE && _child.childNodes.length > 0
                      };
                    });
                  } else if (child.nodeType === child.ELEMENT_NODE && child.tagName === 'TEMPLATE') {
                    firstChild = Array.prototype.filter.call(child.content.childNodes, function (templateChild) {
                      switch (templateChild.nodeType) {
                        case templateChild.COMMENT_NODE:
                          return false;
                    ...
                    return [{
                      ...
                      templateNode: null,
                      type: child.nodeType,
                      text: child.nodeType === child.TEXT_NODE ? child.textContent : null,
                      childTextNode: child.nodeType === child.ELEMENT_NODE && child.childNodes.length > 0
                    }];
                  }
                  ...
                  for (var i = 0; i < currentList.length; i++) {
                    ...
                    } else if (current.type === current.node.TEXT_NODE && current.childTextNode) {
                      // in template node
                      n++;
                      prev.text[0] += '{' + n + '}';
                      textContent = current.node.textContent;
                ...
                templateText.setAttribute('lang', '{{effectiveLang}}'); // insert i18n-format
                ...
                Array.prototype.forEach.call(templateTextParams.params, function (param) {
      ...
      case node.TEXT_NODE:
        ...
        if (text.length === 0 || text.match(/^\s*$/g)) {
          ...
          if (this._isCompoundAnnotatedText(text)) {
            ...
            n = 0;
            messageId = this._generateMessageId(path, id);
            templateTextParams = Array.prototype.map.call([node], function (child) {
              return this._compoundAnnotationToSpan(child).map(function (_child) {
                return {
                  node: _child,
                  type: _child.nodeType,
            ...
            this._setBundleValue(bundle, messageId, text); // replace textContent with annotation


            node.textContent = '{{text.' + messageId + '}}';

            if (!id) {//span.id = messageId;
      ...
      default:
        isWhiteSpace = true; // comment node, etc.
  ...
  _isCompoundAnnotatedText: function (text) {
    return !text.trim().match(/^({{[^{}]*}}|\[\[[^\[\]]*\]\])$/) && !!text.match(/({{[^{}]*}}|\[\[[^\[\]]*\]\])/);
  },
  ...
  _compoundAnnotationToSpan: function (node) {
    ...
    if (node.textContent.match(/({{[^{}]*}}|\[\[[^\[\]]*\]\])/)) {
      result = node.textContent.match(/({{[^{}]*}}|\[\[[^\[\]]*\]\]|[^{}\[\]]{1,}|[{}\[\]]{1,})/g).reduce(function (prev, current) {
        if (current.match(/^({{[^{}]*}}|\[\[[^\[\]]*\]\])$/)) {
          prev.push(current);
          prev.push('');
        } else {
  ...
  tr: function (key, table) {
    if (table) {
      ...
      return typeof this.text === 'object' && typeof key !== 'undefined' && typeof this.text[key] !== 'undefined' ? this.text[key] : key;
    }
  },
    ...
    if (this.is !== 'i18n-dom-bind') {
      ...
      if (!template) {
        ...
        if (template) {
          ...
          var assetpath = _noTemplateDomModule ? _noTemplateDomModule.assetpath : new URL(document.baseURI).pathname;
          domModule.appendChild(template);
          domModule.setAttribute('assetpath', template.hasAttribute('basepath') ? template.getAttribute('basepath') : template.hasAttribute('assetpath') ? template.getAttribute('assetpath') : assetpath);
  ...
  ready: function () {
    if (this.is === 'i18n-dom-bind') {
      ...
      this._langChanged(this.getAttribute('lang'), undefined); // model per instance
  ...
  attached: function () {
    if (this.observeHtmlLang) {
      this.lang = html.lang; // TODO: this call is redundant
...
BehaviorsStore.I18nBehavior.push({
  get _template() {
    ...
    if (this instanceof HTMLElement && (this.constructor.name ||
    /* name is undefined in IE11 */
    this.constructor.toString().replace(/^function ([^ \(]*)((.*|[\n]*)*)$/, '$1')) === 'PolymerGenerated' && !this.constructor.__finalizeClass) {
