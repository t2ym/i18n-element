...
*/
import { html as litHtml, render, svg } from "./node_modules/lit-html/lit-html.js";
import { _I18nBehavior, I18nControllerBehavior } from "./node_modules/i18n-behavior/i18n-behavior.js"; // Polyfill IE 11
...
const UncamelCase = function UncamelCase(name) {
  ...
  if (!tagName) {
    tagName = name // insert a hyphen between lower & upper
...
const i18nMethods = ((mixin, excludes) => {
  ...
  return result;
})(_I18nBehavior, [// TODO: There should be more unnecessary methods to exclude
'properties', 'listeners', 'bedoreRegister', 'registered', 'created', 'ready', 'attached', 'detached', '_onDomChange', '_updateEffectiveLang']); //console.log('methods', JSON.stringify(methods, null, 2));
...
export const i18n = base => class I18nBaseElement extends mixinMethods(_I18nBehavior, i18nMethods, base) {
   ...
   */
  static get is() {
    return UncamelCase(this.name ||
    /* name is undefined in IE11 */
    this.toString().replace(/^function ([^ \(]*)((.*|[\n]*)*)$/, '$1'));
  }
  /**
  ...
  notifyPath(path, value) {} // this.invalidate()
  ...
  _setText(name, bundle, templateLang) {
    ...
    if (templateLang) {
      I18nControllerBehavior.properties.masterBundles.value[templateLang] = I18nControllerBehavior.properties.masterBundles.value[templateLang] || {};
  ...
  _startMutationObserver() {
    ...
    this._htmlLangObserver.observe(this._html = I18nControllerBehavior.properties.html.value, {
      attributes: true
    });
...
export const html = (strings, ...parts) => {
  ...
  if (cachedTemplate) {
    ...
    originalHtml += strings[i + offset]; //console.log('original html: ', originalHtml);
    ...
    preprocessedPartsGenerator = new Function('element', 'parts', 'text', 'model', 'effectiveLang', `return [${preprocessedPartsExpressions.join(',')}]`); //console.log('preprocessedPartsGenerator', preprocessedPartsGenerator.toString());
  ...
  preprocessedParts = preprocessedPartsGenerator(element, parts, text, text.model, element.effectiveLang || element.lang); //console.log('preprocessed: strings ', preprocessedStrings, 'parts ', JSON.stringify(preprocessedParts, null, 2));
