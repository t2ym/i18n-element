...
`,
  ...
  properties: {
    ...
    paramAttribute: {
      type: String,
      value: (function () { return Element ? 'slot' : 'param'; })(),
  ...
  _templateMutated: function (mutations) {
    mutations.forEach(function(mutation) {
      ...
      case 'characterData':
        ...
        if ((Element ? mutation.target : dom(mutation.target)).parentNode.tagName.toLowerCase() !== 'i18n-number' ||
            typeof (Element ? mutation.target : dom(mutation.target)).parentNode.formatted !== 'undefined') {
          this.render();
        }
        break;
      case 'attributes':
